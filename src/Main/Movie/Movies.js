...
const Movies = (props) => {
  ...
  const handlePlayClick = async() => {
    ...
    setVideoPlaying({isplaying:true,url:`http://${props.credentials.domain}/movie/${props.credentials.username}/${props.credentials.password}/${movieDetails.movie_data.stream_id}.${movieDetails.movie_data.container_extension}`});
  };
  ...
  const transformCategories = async (categories) => {
    ...
    try {
      const response = await axios.get(
        `http://${props.credentials.domain}/player_api.php?username=${props.credentials.username}&password=${props.credentials.password}&action=get_vod_streams`
      );
  ...
  useEffect(() => {
    const getMovies = async () => {
      try {
        const response = await axios.get(
          `http://${props.credentials.domain}/player_api.php?username=${props.credentials.username}&password=${props.credentials.password}&action=get_vod_categories`
        );
  ...
  const handleMovieClick = async (movie) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://${props.credentials.domain}/player_api.php?username=${props.credentials.username}&password=${props.credentials.password}&action=get_vod_info&vod_id=${movie.stream_id}`
      );
