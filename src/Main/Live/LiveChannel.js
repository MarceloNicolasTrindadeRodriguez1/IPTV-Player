...
const LiveChannel = (props) => {
  ...
  const transformCategories = async (categories) => {
    ...
    await axios.get(`http://${props.credentials.domain}/player_api.php?username=${props.credentials.username}&password=${props.credentials.password}&action=get_live_streams`)
      .then(list => {
  ...
  useEffect(() => {
    const getChannels = async () => {
      await axios.get(`http://${props.credentials.domain}/player_api.php?username=${props.credentials.username}&password=${props.credentials.password}&action=get_live_categories`)
        .then(async response => {
          ...
          selectedChannel ? 
            <div style={{ width: "50%", padding: "10px" }}>
              <VideoPlayer
                ...
                videoUrl={`http://${props.credentials.domain}/live/${props.credentials.username}/${props.credentials.password}/${selectedChannel.stream_id}.m3u8`} />
            </div>
           : (
            <></>
          )
